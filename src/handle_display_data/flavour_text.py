...
class FlavourText:
    ...
    def _get_direction_of_niche_text(self):
        ...
        logic_dict = {
            self.means[shortest_term]
            > self.means[
                longest_term
            ]: f"increased from {round(self.means[longest_term], 1)} to {round(self.means[shortest_term], 1)}",
            self.means[shortest_term] == self.means[longest_term]: "stayed the same",
            self.means[shortest_term]
            < self.means[
                longest_term
    ...
    def _generate_flavour_text(self):
        ...
        for term in self.terms:
            ...
            most_pop_texts.append(
                f"{TEXT.MOST_POPULAR} {self.most_popular_artists[term]}"
            )
            least_pop_texts.append(
                f"{TEXT.LEAST_POPULAR} {self.least_popular_artists[term]}"
            )
    ...
    def _get_flavour_text_elements(self):
        ...
        for index in number_of_terms:
            for key in flavour_text_dict:
                ...
                display_text_list.append(
                    plt.gcf().text(
                        x=GRAPH.FLAVOUR["x"],
                        y=GRAPH.FLAVOUR["y_list"][count],
                        s=flavour_text_dict[key][index],
                        weight=weight,
