...
class ConvertDataTests(TestCase):
    ...
    def test_row_extraction(self):
        ...
        self.assertEqual(
            list_of_rows[0],
            ["short_term_band_1", 80, "short_term"],
            "Row should be name, popularity and term of corresponding entry in dummy_spotify_data.",
        )
        self.assertEqual(
            list_of_rows[3],
            ["medium_term_band_2", 65, "medium_term"],
            "Row should be name, popularity and term of corresponding entry in dummy_spotify_data.",
        )
        self.assertEqual(
            list_of_rows[4],
            ["long_term_band_1", 60, "long_term"],
            "Row should be name, popularity and term of corresponding entry in dummy_spotify_data.",
        )
...
class ConnectionTests(TestCase):
    ...
    dummy_insertion_data = [
        ["short_term_band", "popularity_value", "short_term"],
    ...
    def test_create_close_connection(self):
        ...
        self.assertTrue(
            path.isfile("dummy_database.db"),
            "dummy_database.db should exist if connection established.",
        )
        connection.close()
        connection = None
        self.assertIsNone(
            connection, "connection should be None after closing and setting to None."
        )
    ...
    def test_step_a_create_table(self):
        ...
        self.assertTrue(
            stat(self.database).st_size != 0, "if table, file size should be > 0"
        )
    ...
    def test_step_c_data_inserted_into_table(self):
        ...
        self.assertEqual(
            fetched_results[0], (1, "short_term_band", "popularity_value", "short_term")
        )
        self.assertEqual(
            fetched_results[2], (3, "long_term_band", "popularity_value", "long_term")
        )
